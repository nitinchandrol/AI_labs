...
struct MinHeap{
	...
	void deleteMin(){
		...
		if(ch == '0'){
			...
			state_map[start_node->state->state_id] = start_node;
			delete last_node;
		} else {			
	...
	void remove(State* state){
		if(DEBUG){
			cout << "deleting "<< state->state_id <<" from ";
			this->print(start_node);
			cout << endl;
		}
		string bm = getBitmap(size);
		...
		Node* last_node = start_node;
		char ch;
		Node* this_node = state_map[state->state_id];
		while(next != ""){
			ch = next[0];				
			if(ch == '0'){
				last_node = last_node->left;
		...
		state_map.erase(state->state_id);
		if(last_node == start_node){			
			delete start_node;
			start_node = NULL;
			this->size = 0;
		...
		if(ch == '0'){
			last_node->parent->left = NULL ;
			this_node->state = last_node->state;
			state_map[last_node->state->state_id] = this_node;
			delete last_node;
		} else {			
			last_node->parent->right = NULL ;
			this_node->state = last_node->state;
		...
		if(current_node != start_node && current_node->state->getFcost() < current_node->parent->state->getFcost()){
			balanceUp(current_node);
		} else {
			balanceDown(current_node);
		}
		if(DEBUG){
			this->print(start_node);
			cout << endl;
	...
	void balanceUp(Node* new_node){
		if(DEBUG){
			cout << "balancing up" << endl;
		}		
	...
	void balanceDown(Node* current_node){
		if(DEBUG){
			cout << "balancing down" << endl;
		}
